...
if (!isset($_SESSION['phone'])) {
    ...
    $user_check = $conn->query("SELECT phone, name, profile_photo FROM users WHERE id = $uid");
    if ($user_check->num_rows > 0) {
        $u_data = $user_check->fetch_assoc();
        $_SESSION['phone'] = $u_data['phone'];
        $_SESSION['name'] = $u_data['name'];
        $profile_photo = $u_data['profile_photo'];
    }
} else {
    // Still need to fetch profile_photo for display
    $uid = $_SESSION['user_id'];
    $user_check = $conn->query("SELECT profile_photo FROM users WHERE id = $uid");
    $u_data = $user_check->fetch_assoc();
    $profile_photo = $u_data['profile_photo'];
}
