...
<body class="font-outfit bg-cream text-gray-800 overflow-x-hidden">
    ...
    <nav class="fixed top-0 w-full z-50 glass-nav transition-all duration-300">
        <div class="container mx-auto px-6 py-4 flex items-center">
            ...
            <div class="hidden md:flex items-center gap-8 ml-16 pl-8 border-l border-gold/20">
        ...
        <div class="container mx-auto px-6 relative z-10 text-center md:text-left">
            <div class="max-w-3xl">
                ...
                <div class="flex flex-col sm:flex-row gap-4" data-aos="fade-up" data-aos-delay="600">
                    <a href="#rooms" class="bg-gold text-white px-8 py-3.5 rounded-full font-bold text-sm uppercase tracking-widest hover:bg-white hover:text-gold transition-all duration-300 shadow-2xl">
                        Explore Rooms
                    </a>
                    <a href="<?php echo isset($_SESSION['user_id']) ? '#rooms' : 'register.html'; ?>" class="bg-transparent border-2 border-white/50 text-white px-8 py-3.5 rounded-full font-bold text-sm uppercase tracking-widest hover:bg-white/10 transition-all duration-300 backdrop-blur-sm">
                        Book Now
                    </a>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                ...
                if ($res) {
                    ...
                    while($row = $res->fetch_assoc()) {
                        ...
                        $seed = (int)$row['room_number'];
                        $defaultImg = $pool[$seed % count($pool)];
                        $displayImg = !empty($row['image']) ? $row['image'] : $defaultImg;
